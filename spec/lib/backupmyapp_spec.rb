...
describe Backupmyapp do
  ...
  describe "HTTP methods" do
    before(:each) do
      @backuper = Backupmyapp.new
      @http_example = Net::HTTP.post_form(URI.parse("http://google.com/"), {})
    end
    
    it "should send correct request to server without options" do
      Net::HTTP.should_receive(:post_form).with(URI.parse("http://backupmyapp.com/backups/init/backup/#{@key}"), {}).and_return(@http_example)
      @backuper.post("init/backup")
    end
    
    it "should send correct request to server with options" do
      Net::HTTP.should_receive(:post_form).with(URI.parse("http://backupmyapp.com/backups/init/backup/#{@key}"), { :directories => "app public"}).
      and_return(@http_example)
      @backuper.post("init/backup", {:directories => "app public"})
    end
  end
